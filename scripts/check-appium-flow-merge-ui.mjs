...
try {
  ...
  const enlarge = page.getByRole('button', { name: '放大', exact: true });
  await expect(enlarge).toHaveText('');
  await enlarge.hover();
  await expect(page.getByRole('tooltip').filter({ hasText: /^放大$/ })).toBeVisible();
  await enlarge.click();
  const overview = page.getByRole('dialog', { name: '流程总览' });
  await overview.getByRole('button', { name: '批量删除', exact: true }).click();
  await overview.locator('.appium-flow-step-card').filter({ hasText: '独有操作' }).click();
  await overview.locator('.appium-flow-step-card').filter({ hasText: '公共后续' }).click();
  await expect(page.getByText('批量删除只能选择连续节点', { exact: true })).toBeVisible();
  await expect(page.getByText('批量复制只能选择连续节点', { exact: true })).toHaveCount(0);
  await overview.getByRole('button', { name: '取消', exact: true }).click();
  ...
  await page.getByRole('button', { name: '放大', exact: true }).click();
  await overview.getByRole('button', { name: '批量删除', exact: true }).click();
  await expect(overview.getByRole('button', { name: '删除选中', exact: true })).toBeDisabled();
  await card('判断勾选').click();
  await expect(overview.locator('.appium-flow-step-shell--delete-selected')).toHaveCount(2);
  await expect(card('独有操作')).toHaveCSS('background-color', 'rgb(184, 50, 50)');
  await expect(card('独有操作').locator('strong')).toHaveCSS('color', 'rgb(255, 255, 255)');
  await card('重复操作').click();
  await expect(overview.locator('.appium-flow-step-shell--delete-selected')).toHaveCount(3);
  await card('重复操作').click();
  await overview.getByRole('button', { name: '删除选中', exact: true }).click();
  const confirmation = page.getByRole('dialog', { name: '批量删除', exact: true });
  await expect(confirmation).toContainText('2 个节点');
  await confirmation.getByRole('button', { name: '取消', exact: true }).click();
  await expect(overview.locator('.appium-flow-step-shell--delete-selected')).toHaveCount(2);
  await overview.getByRole('button', { name: '删除选中', exact: true }).click();
  await confirmation.getByRole('button', { name: '删除', exact: true }).click();
  await expect(overview.locator('.appium-flow-step-card')).toHaveCount(1);
  await expect(card('重复操作')).toBeVisible();
  await expect(overview.getByRole('button', { name: '批量删除', exact: true })).toBeVisible();
